...
namespace GameTroopers.UI
{
    ...
    public class Menu : MonoBehaviour
    {
        ...
        public Menu()
        {
            ...
            m_showAnimType = AnimType.Instant;
        ...
        internal bool IsGoBackEnabled
        {
            get { return m_isGoBackEnabled; }
        }

        internal bool ShouldSwallowFocus
        {
            get { return m_shouldSwallowFocus; }
        }
        ...
        {
            get { return m_showAction; }

            set
            {
                if (m_showAnimType == AnimType.Custom)
                {
                    m_showAction = value;
                }
                else
        ...
        {
            get { return m_hideAction; }

            set
            {
                if (m_hideAnimType == AnimType.Custom)
                {
                    m_hideAction = value;
                }
                else
        ...
        internal void OnLoadMenu(MenuManager menuManager)
        {
            ...
            if (m_showAnimType != AnimType.Custom)
            {
                m_showAction = AnimationHelper.GetDefaultAnimation(m_showAnimType);
            }

            if (m_hideAnimType != AnimType.Custom)
            {
                m_hideAction = AnimationHelper.GetDefaultAnimation(m_hideAnimType);
            }
        ...
        protected internal void SetInteractableButtons(bool enable)
        {
            if (m_inputEnabled != enable)
            {
                for (int i = 0; i < m_selectables.Length; ++i)
                {
                    m_selectables[i].interactable = enable;
                }
        ...
        private IEnumerator ShowAnimationCoroutine(bool shouldPlayAnimation)
        {
            ...
            {
                yield return ShowAction(this, GetMenuOffScreenPosition(), GetMenuOnScreenPosition(), m_animationDuration);
            }
            else
            {
        ...
        private IEnumerator HideAnimationCoroutine(bool shouldPlayAnimation)
        {
            ...
            if (shouldPlayAnimation)
            {
                yield return HideAction(this, GetMenuOnScreenPosition(), GetMenuOffScreenPosition(), m_animationDuration);
            }
            else
            {
                rectTransform.anchoredPosition = GetMenuOffScreenPosition();
